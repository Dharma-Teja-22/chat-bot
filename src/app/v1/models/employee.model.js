...
class Employee {
  static async create(data) {
    ...
    try {
      const sql = `INSERT INTO ${tableName} (employee_id, first_name, last_name, email, phone_number, hire_date, salary, manager_id, department_id)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
  ...
  static async assign(empData) {
    try {
      const pool = await poolPromise;
      const { employeeId, projectId, role, hoursWorked, departmentId } = empData;
    ...
    try {
      ...
      if (empProject[0].affectedRows !== 0) {
        ...
        if (emps[0].length > 0) {
          ...
          await pool.query(
            `update ${tableName} set manager_id = ? where manager_id = ?`,
            [mid, employeeId]
          );
        }
